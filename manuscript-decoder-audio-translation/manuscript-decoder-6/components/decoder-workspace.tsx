...
import { downloadFile, manuscripts, translationLocale, translationLanguages, type Manuscript, type Script, type TranslationLanguage } from '@/lib/manuscripts'
import { TranslationLanguageSelect } from '@/components/translation-language-select'
import { AudioSpeakerButton } from '@/components/audio-speaker-button'
import { speak, stopSpeech } from '@/lib/text-to-speech'
...
export function DecoderWorkspace({ initial, initialComplete = false, onSave }: { initial?: Manuscript; initialComplete?: boolean; onSave: (item: Manuscript) => void }) {
  ...
  function listen() {
    if (!item) return
    speak(item.text, 'Hindi', (error) => toast.error(error))
  ...
  return <div className="flex flex-col gap-7">
    ...
    {phase === 'upload' ? <UploadZone onSample={chooseSample} onUpload={(image, name) => { setItem({ ...manuscripts[0], id: crypto.randomUUID(), title: name, image, translations: { ...manuscripts[0].translations }, date: new Date().toISOString().slice(0, 10) }); setUploaded(true); setPhase('ready') }} /> : item && <>
      ...
      <Tabs defaultValue="results">
        {complete && <TabsList className="no-print mb-4"><TabsTrigger value="results"><FileText />Results</TabsTrigger><TabsTrigger value="audio"><Volume2 />Audio translations</TabsTrigger><TabsTrigger value="compare"><Layers3 />Compare versions</TabsTrigger></TabsList>}
        ...
        <TabsContent value="audio"><section className="panel overflow-hidden"><h2 className="panel-heading"><span className="flex items-center gap-2"><Globe2 className="size-4 text-primary" />Listen to translations in all languages</span></h2><div className="p-5"><div className="mb-5"><p className="text-sm text-muted-foreground mb-4">Click the speaker button to hear the translation in each language:</p></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{translationLanguages.map(lang => <div key={lang.name} className="flex items-center justify-between rounded-lg border border-border bg-card/50 p-4 hover:border-primary/50 transition-colors"><div className="flex-1"><p className="text-sm font-medium text-foreground">{lang.name}</p><p className="text-xs text-muted-foreground mt-1 line-clamp-2">{item.translations[lang.name as TranslationLanguage]}</p></div><AudioSpeakerButton text={item.translations[lang.name as TranslationLanguage]} language={lang.name as TranslationLanguage} className="ml-3 shrink-0" /></div>)}</div><p className="text-xs text-muted-foreground mt-6 pt-4 border-t border-border">Note: Audio uses your browser's text-to-speech. Quality depends on system language voices installed. Some languages may not be available.</p></div></section></TabsContent>
