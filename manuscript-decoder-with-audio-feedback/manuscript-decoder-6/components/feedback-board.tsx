...
import { MessageSquareText, Send, Star, ThumbsUp, Trash2, Globe2 } from 'lucide-react'
...
export function FeedbackBoard() {
  ...
  const [selectedTranslationLang, setSelectedTranslationLang] = useState<TranslationLanguage>('English')
  ...
  return <div className="flex flex-col gap-8">
    ...
    <section className="panel overflow-hidden">
      ...
      {entries.length === 0 ? <Empty>
        ...
        {entries.map(entry => <li key={entry.id} className="flex flex-col gap-4 p-5">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <AudioSpeakerButton text={entry.message} language="English" />
            <button onClick={() => setExpandedFeedback(expandedFeedback === entry.id ? null : entry.id)} className="flex items-center gap-1.5 text-xs text-primary hover:text-primary/80"><Globe2 className="size-3.5" />{expandedFeedback === entry.id ? 'Hide translations' : 'Hear in other languages'}</button>
          </div>
          {expandedFeedback === entry.id && <div className="mt-3 space-y-3 rounded-lg bg-primary/5 p-4">
            <p className="text-xs text-muted-foreground">Listen to this feedback in different languages:</p>
            <div className="flex flex-wrap gap-2">
              {translationLanguages.map(lang => <div key={lang.name} className="flex items-center gap-1.5 rounded-lg bg-card px-3 py-2">
                <span className="text-xs font-medium">{lang.name}</span>
                <AudioSpeakerButton text={entry.message} language={lang.name as TranslationLanguage} />
              </div>)}
            </div>
            <p className="text-[11px] text-muted-foreground">Note: Audio uses browser's text-to-speech. Pronunciation depends on your system's installed language voices.</p>
          </div>}
